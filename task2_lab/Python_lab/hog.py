...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    """
    # BEGIN PROBLEM 4
    gain = take_turn(num_rolls, player_score, opponent_score, dice=six_sided)
    total = score + gain
    return sus_points(total)
    # END PROBLEM 4
...
def play(strategy0, strategy1, update, score0=0, score1=0, dice=six_sided, goal=GOAL):
    ...
    # BEGIN PROBLEM 5
    while score0 < goal and score < goal:
        if who == 0:
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    def strategy(my_score,opp_score):
        return n
    return strategy
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    first = strategy(0,0)
    for my in range(goal):
        for opp in range(goal):
            if strategy(my,opp) != first:
                return False
    return True
    # END PROBLEM 7
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    ten = opponent_score//10
    one = opponent_score%10
    if abs(ten - one) == 2 and (ten * one) % 3 ==0 :
        sus_gain = 3
    else :
        sus_gain = 0
    total_gain = ten + sus_gain
